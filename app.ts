...
// console.log(chalk.cyan.italic("\n\t Welcome to the Quiz app (by Aijaz)\t\n"));
...
async function askQuestions():Promise<void>{
  ...
  for (const q of questions) {
    ...
    if (answer.response === q.correctAnswer) {
      ...
      console.log(
        ` Wrong! The correct answer is ${chalk.bold.green(q.correctAnswer)}`
      );
      wrongAnswers++;
    }
  }
  ...
  console.log(
    chalk.bold(
      `${chalk.green("Correct Answers:")} ${correctAnswers}\n${chalk.red(
        "Wrong Answers:"
      )} ${wrongAnswers}\n${chalk.yellowBright("Total Questions:")} ${
        questions.length
      }`
    )
  );
